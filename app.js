...
listApiRouter.post('/', function(req, resp, next) {
  var fields = getFields(LIST_FIELDS_NEW, req.body);
  fields.pos = parseInt(fields.pos);
  if (! isValid(LIST_FIELDS_NEW, fields)) {
    resp.status(400).end();
  } else {
...
listApiRouter.delete('/:id', function(req, resp, next) {
  var id = parseInt(req.params.id.match(/\d+/g)[0])
  var result = storage.getOne('list', id);
  if (!result) {
    resp.status(400).end();
  } else {
    console.log('Delete list', result);
    let bool = storage.del('list', id);
    // resp.json(bool)
  }
});
